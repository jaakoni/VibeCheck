...
import com.vibecheck.events.network.TicketmasterEvent
...
class TicketmasterRepository(private val apiKey: String) {
    ...
    suspend fun fetchEvents(city: String, startDateTime: String? = null, endDateTime: String? = null): List<Event> {
        ...
        return ticketmasterEvents.map { tmEvent ->
    ...
    suspend fun fetchEventById(id: String): Event {
        ...
        return mapToEvent(tmEvent, tmEvent._embedded?.venues?.firstOrNull()?.city?.name ?: "Atlanta")
    }
    ...
    private fun mapToEvent(tmEvent: TicketmasterEvent, city: String): Event {
        ...
        val venueTimezone = venue?.timezone

        // Parse date using venue timezone or fallback to device default
        val localDate = tmEvent.dates?.start?.localDate
        val localTime = tmEvent.dates?.start?.localTime
        val startTimestamp = parseTmDate(localDate, localTime, venueTimezone)
        ...
        val mappedCategory = when {
            ...
            else -> EventCategory.COMMUNITY_FESTIVALS // Neutral fallback
        }

        val performerName = tmEvent._embedded?.attractions?.firstOrNull()?.name
        val promoterName = tmEvent.promoters?.firstOrNull()?.name
        val hostName = performerName ?: promoterName ?: venueName.takeIf { it != "Unknown Venue" } ?: "Event Organizer"

        val attractionTags = tmEvent._embedded?.attractions?.mapNotNull { it.name } ?: emptyList()
        val allTags = (listOf(segmentName, "VibeCheck Choice") + attractionTags).filter { it.isNotBlank() }.distinct()
    ...
    private fun parseTmDate(localDate: String?, localTime: String?, tzId: String?): Long {
        ...
        return try {
            ...
            format.timeZone = tzId?.let { java.util.TimeZone.getTimeZone(it) } ?: java.util.TimeZone.getDefault()
